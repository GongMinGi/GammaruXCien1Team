...
public class AttackEffectDefTests
{
    ...
    public void Expand_ProducesCorrectEffectSequence()
    {
        ...
        Assert.AreEqual(EffectType.Cast, effects[0].Type);
        Assert.AreEqual(card, effects[0].SourceCard);
        Assert.AreEqual(DamageElement.Neutral, effects[0].Element);

        Assert.AreEqual(EffectType.DealDamage, effects[1].Type);
        Assert.AreEqual(card, effects[1].SourceCard);
        Assert.AreEqual(DamageElement.Neutral, effects[1].Element);
        Assert.AreEqual(5, effects[1].BaseValue);
        Assert.AreEqual(1.5f, effects[1].SpellPowerCoefficient);
        Assert.AreEqual(2, effects[1].AdditionalEffectValue);
